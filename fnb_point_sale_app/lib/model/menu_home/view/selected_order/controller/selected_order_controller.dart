...
class SelectedOrderController extends HomeBaseController {
  ...
  getCartItemKot() async {
    ...
    debugPrint("## new Kot ## ${jsonEncode(cartItemSelectKot)}");
        ...
        CartItem mCartItemSelect =
          ...
          if (value) {
            value = element.mMenuItemData?.menuItemIDP.toString() ==
                mCartItemHistory.mMenuItemData?.menuItemIDP.toString();
          }

          if (value) {
            value = element.mSelectVariantListData?.variantIDP.toString() ==
                mCartItemHistory.mSelectVariantListData?.variantIDP.toString();
          }

          if (value) {
            value = element.getModifierString() ==
                mCartItemHistory.getModifierString();
          }

          if (value) {
            value = element.textRemarks.toString() ==
                mCartItemHistory.textRemarks.toString();
          }

          return value;
        }, orElse: () => CartItem());
        // debugPrint("mCartItemSelect ${jsonEncode(mCartItemSelect)}");
        if (mCartItemSelect.mMenuItemData == null) {
          CartItem mCartItemKot = CartItem.fromJson(mCartItemHistory.toJson());
          mCartItemKot.count = (-1) * (mCartItemHistory.count);
          mCartItemKot.cancelOrder = mCartItemKot.count < 0;
          cartItemKot.add(mCartItemKot);
        } else if (mCartItemSelect.count != mCartItemHistory.count) {
          CartItem mCartItemKot = CartItem.fromJson(mCartItemSelect.toJson());
          mCartItemKot.count = mCartItemSelect.count - mCartItemHistory.count;
          mCartItemKot.cancelOrder = mCartItemKot.count < 0;
          cartItemKot.add(mCartItemKot);
        }
      }
    ...
    debugPrint("## cartItemKot ## ${jsonEncode(cartItemKot)}");
    cartItemKot.addAll(cartItemSelectKot.toList());
  }
