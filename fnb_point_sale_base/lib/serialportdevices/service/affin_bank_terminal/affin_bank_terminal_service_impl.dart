...
import '../../../locator.dart';
import 'affin_bank_terminal_service.dart';
...
class AffinBankTerminalServiceImpl with AffinBankTerminalService {
  final tag = "AffinBankTerminalService";
  ...
  Future<bool> echoTerminalTest() async {
    ...
    MyLogUtils.logDebug("$tag, echoTerminalTest : ");
    var localApi = locator.get<SerialPortDevicesApi>();
    var device = await localApi.getAffinPaymentTerminalDevice();
    MyLogUtils.logDebug("$tag, echoTerminalTest device: ${device?.name ?? ""}");
    MyLogUtils.logDebug(
        "$tag, echoTerminalTest device: ${device?.address ?? ""}");
    ...
    if (device != null) {
      ...
      try {
        ...
        List<int> presentationHeader =
            createPresentationHeader(transactionCode: "20", responseCode: "00");
        List<int> fieldData =
            createFieldData(fieldCode: "00", data: "00000000000000000000");
        fieldData.addAll(
            createFieldData(fieldCode: "66", data: "00020230620090912971"));
        fieldData
            .addAll(createFieldData(fieldCode: "40", data: "000000000001"));
        ...
        //  MyLogUtils.logDebug("\n‘60’  MyLogUtils.logDebug RECEIPT\n");
        // List<int> transportHeader = createTransportationHeader();
        // List<int> presentationHeader =
        // createPresentationHeader(transactionCode: "60", responseCode: "00");
        // List<int> fieldData =
        // createFieldData(fieldCode: "00", data: "00000000000000000000");
        // fieldData.addAll(createFieldData(fieldCode: "60", data: "31313131313131313131323232323232323232320A0D33333333333333333333343434343434343434340A0D35353535353535353535363636363636363636360A0D37373737373737373737383838383838383838380A0D"));
        // List<int> message =
        // constructMessage(transportHeader, presentationHeader, fieldData);
        // String inputCommand = listToHexString(message);
        ...
        await _readAndAcknowledge(serialPortDevice, message, 9,
            (value, mUint8List) {
          MyLogUtils.logDebug(
              type: LogType.AFFIN, "   readResultAsString: $value");
    ...
    if (device == null) {
      ...
      try {
        ...
        await _readAndAcknowledge(serialPortDevice, message, 9,
            (value, mUint8List) {
          MyLogUtils.logDebug(
              type: LogType.AFFIN, "xx   readResultAsString: $value");
          if (value.toString().trim().toUpperCase() != ("ACK")) {
            closeTheDevice(serialPortDevice);
            ...
            for (var element in mListValue) {
              if (element.trim().length > 3) {
                ...
                if (element.trim().substring(0, 2).toUpperCase().trim() ==
                    "D4") {
                  cardType = getPaymentCardFromAffinCode(element.trim().substring(2).trim());
                }
  ...
  Future<void> _readAndAcknowledge(SerialPort portDevice, List<int> data,
    ...
    MyLogUtils.logDebug(
        type: LogType.AFFIN, "$tag, _readAndAcknowledge data : $data ");
    ...
    reader.stream.listen((data) {
      String readResultAsString = affinBankConvertUInt8ListToString(data);
      MyLogUtils.logDebug(
          type: LogType.AFFIN,
