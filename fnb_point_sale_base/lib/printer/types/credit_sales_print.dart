...
// import 'package:fnb_point_sale_base/data/local/database/promoters/promoters_local_api.dart';
// import 'package:fnb_point_sale_base/data/local/database/sale/model/cart_summary.dart';
// import 'package:fnb_point_sale_base/jakel_base.dart';
// import 'package:fnb_point_sale_base/printer/printer_helper.dart';
// import 'package:fnb_point_sale_base/restapi/sales/model/history/save_sale_response.dart';
// import 'package:fnb_point_sale_base/sale/sale_helper.dart';
// import 'package:fnb_point_sale_base/viewmodel/base_view_model.dart';
// import 'package:pdf/widgets.dart' as pw;
//
// import '../../database/companyconfig/company_config_local_api.dart';
// import '../../database/user/user_local_api.dart';
// import '../../locator.dart';
// import '../../restapi/counters/model/get_counters_response.dart';
// import '../../restapi/me/model/current_user_response.dart';
// import '../../restapi/promoters/model/promoters_response.dart';
// import '../../restapi/sales/model/history/sales_response.dart';
// import '../../utils/num_utils.dart';
//
// Future<bool> printCreditSaleData(String? title, Sale sale, bool duplicateCopy,
//     bool voidSale, SaleTye saleTye) async {
...
//     widgets.add(
//         pw.Center(child: pw.Text('CREDIT SALE', style: getNormalTextStyle())));
//     widgets.add(pw.Container(height: 3));
//     widgets.add(mySeparator());
//   }
//
...
//     widgets.addAll(
//         getVoucherUsed(sale.usedVoucher, sale.member)); //  Voucher used
//     widgets.add(pw.Container(height: 5));
//   }
//
//   if (sale.member != null) {
...
//   }
//
//   final isCartPromoCodeApplied = sale.promoCode != null && (sale.promoCode?.isNotEmpty ?? false);
//
//   ///SaleItemDiscountsList
//   if (isCartPromoCodeApplied || isSaleItemDiscountsList(sale.saleItems??[]).isNotEmpty) {
//     widgets.addAll(saleItemDiscountsListWidget(sale));
//     widgets.add(pw.Container(height: 10));
//   }
//
//   if (!voidSale) {
...
//
//   /// If EInvoice enable then show QR
//   if(companyConfig?.showEInvoiceQrOnReceipt ?? false) {
//     widgets.add(pw.Container(height: 5));
//     widgets.add(qrCodeWidget(await getEInvoiceUrl(sale.offlineSaleId ?? '', 'SS')));
//     widgets.add(pw.Container(height: 5));
//     widgets.add(mySeparator());
//     widgets.add(pw.Container(height: 3));
//   }
...
// }
//
// List<pw.Widget> saleItemDiscountsListWidget(Sale sale) {
//   List<pw.Widget> widgets = List.empty(growable: true);
//   widgets.add(pw.Align(
...
//   widgets.add(pw.Container(height: 5));
//
//   /// If cart promocode is applied
//   if(sale.promoCode != null && (sale.promoCode?.isNotEmpty ?? false)) {
//     widgets.add(pw.Align(
//           alignment: pw.Alignment.centerLeft,
//           child: pw.Text(
//               ' ${sale.promoCode ?? ''}',
...
//
//   String usePromo  = "";
//   isSaleItemDiscountsList(sale.saleItems??[]).forEach((element) {
//     if(usePromo!=(element.promoCode ?? '')) {
//       widgets.add(pw.Align(
//           alignment: pw.Alignment.centerLeft,
//           child: pw.Text(
//               ' ${element.promoCode ?? ''}',
//               style: getNormalTextStyle())));
//       usePromo = (element.promoCode ?? '');
//     }
//   });
//
//   widgets.add(pw.Container(height: 5));
...
//         '$index. ${element.product?.name ?? noData} (${element.product?.articleNumber ?? ''}) ${element.derivative != null ? '- ${element.derivative?.name}' : ''}'));
//
...
//           child: pw.Align(
//             alignment: pw.Alignment.centerRight,
//             child: pw.Text(getOnlyReadableAmount(ordersItem.derivative != null ? getOnlyReadableAmount(ordersItem.derivative?.pricePaidOfDerivative) : getTotalPricePaid(ordersItem)),
//                 style: getNormalTextStyle()),
//           ),
//         )),
//   ]);
...
//       "SUB TOTAL",
//       getOnlyReadableAmount(getCreditSubTotal(sale)),
//     ));
//   }
//
//   tableRows.add(amountsRowBoldWidget(
//       "DISCOUNT", getOnlyReadableAmount(getTotalDiscountCreditAmount(sale))));
//
...
//   tableRows.add(amountsRowBoldWidget(
//     "TOTAL",
//     getOnlyReadableAmount(getTotalCreditAmount(sale)),
//   ));
//
//   tableRows.add(amountsRowBoldWidget(
//     "TOTAL PAID",
//     getOnlyReadableAmount(getCrediTotalPaid(sale)),
//   ));
//
...
//   }
//
//   if ((sale.creditPendingAmount ?? 0) > 0) {
//     tableRows.add(amountsRowBoldWidget(
//         "------------------------", "------------------------"));
//     tableRows.add(amountsRowBoldWidget(
//       "BALANCE",
//       getOnlyReadableAmount(sale.creditPendingAmount ?? 0),
//     ));
//     tableRows.add(amountsRowBoldWidget(
//         "------------------------", "------------------------"));
//   }
//
//   if (!isLayawaySaleType(sale.status) && !isCreditSaleType(sale.status)) {
//     tableRows.add(amountsRowBoldWidget(
//       "CHANGE DUE",
...
//
//   sale.payments?.forEach((element) {
//     if (double.parse(getOnlyReadableAmount(element.amount ?? 0)) > 0.0) {
...
//
//   if ((user?.redeemPointsToProduct ?? 0) > 0) {
//     pwWidgets.add(getLeftRightText1("Redeem Points to Product",
//         '-${getInValue(user?.redeemPointsToProduct ?? 0)}'));
//   }
//
//   if((user?.redeemPoints ?? 0) > 0) {
//     pwWidgets.add(getLeftRightText1(
//         "Redeemed Points", '-${getInValue(user?.redeemPoints ?? 0)}'));
//   }
//
//   if ((user?.earnedPoints ?? 0) >= 0) {
//     pwWidgets.add(getLeftRightText1(
//         "This Sale Points", '${getInValue(user?.earnedPoints ?? 0)}'));
//   }
//
//   pwWidgets.add(getLeftRightText1(
...
//
//   pwWidgets.add(getLeftRightText1(
//       title, getReadableAmount(Constants.currency, voucher?.amount ?? 0)));
//
//   pwWidgets.add(getLeftRightText1(voucher?.number ?? "", ""));
//
//   pwWidgets.add(getLeftRightText1(voucher?.footerNotes ?? "", ""));
//
//   pwWidgets.add(pw.Container(height: 5));
//
//   pwWidgets.add(mySeparator());
