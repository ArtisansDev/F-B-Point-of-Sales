...
import '../../data/mode/order_history/refund_details_response.dart';
import '../../lang/translation_service_key.dart';
import '../../locator.dart';
import '../../utils/date_time_utils.dart';

Future<bool> printSalePaymentRefund(OrderHistoryData mOrderHistoryData,
  ...
  if (mPrinterSettingsData == null) {
    ...
    ///Total
    widgets.add(
        getTotalRow(mOrderHistoryData, mCurrencyData.currencySymbol ?? ''));
    ...
    if (mOrderHistoryData.paymentGatewayNo.toString() == "0") {
      ///Change Due
      widgets.add(getChangeDueRow(
          mOrderHistoryData, mCurrencyData.currencySymbol ?? ''));
    }
    ...
    widgets.add(pw.Container(
        padding: const pw.EdgeInsets.only(left: 2.0),
        child: pw.Text('Refund', style: getBoldTextStyle())));
    widgets.add(pw.Container(height: 3));
    widgets.add(getPaymentRefundRow(
        mOrderHistoryData, mCurrencyData.currencySymbol ?? ''));
    widgets.add(pw.Container(height: 4));
    widgets.add(mySeparator());
    widgets.add(pw.Container(height: 4));
    ...
    if ((mOrderHistoryData.paymentStatus ?? '').toUpperCase() == 'C') {
      ...
      if ((mPrinterSettingsData.enableTitleText ?? false) &&
        ...
        widgets.add(pw.Center(
            child:
                pw.Text('Payment Receipt', style: getBoldTextStyleMedium())));
      }
    }
  ...
  return pw.Row(children: [
    ...
    pw.Expanded(
        ...
        child: pw.Container(
          ...
          child: pw.Align(
            alignment: pw.Alignment.centerRight,
            child: (mOrderHistoryData.paymentGatewayNo.toString() == "0")
                ? pw.Text(
                    '$currencyData ${getDoubleValue(roundToNearestPossible(getDoubleValue(getDoubleValue(mOrderHistoryData.payAmountCash).toStringAsFixed(2)))).toStringAsFixed(2)}',
                    style: getBoldTextStyle())
                : pw.Text(
...
pw.Widget getChangeDueRow(
    OrderHistoryData mOrderDetailList, String currencyData) {
  return pw.Row(children: [
    pw.Expanded(
        flex: 2,
        child: pw.Container(
    ...
    pw.Expanded(
        ...
        child: pw.Container(
          ...
          child: pw.Align(
            alignment: pw.Alignment.centerRight,
            child: pw.Text(
                '$currencyData ${getDoubleValue(roundToNearestPossible(getDoubleValue(getDoubleValue(mOrderDetailList.returnAmountCash).toStringAsFixed(2)))).toStringAsFixed(2)}',
                style: getBoldTextStyle()),
          ),
        )),
  ]);
...
/// payment Refund
pw.Widget getPaymentRefundRow(
    OrderHistoryData mOrderHistoryData, String currencyData) {
  RefundDetails mRefundDetails =
      mOrderHistoryData.refundDetails?.first ?? RefundDetails();
  return pw.Column(children: [
    pw.Row(children: [
      pw.Expanded(
          flex: 2,
          child: pw.Container(
            ...
            child: pw.Align(
              alignment: pw.Alignment.centerLeft,
              child: pw.Text(
                  mRefundDetails.refundedPaymentGatewayNo.toString() == "0"
                      ? 'Cash'
                      : mRefundDetails.refundedPaymentGatewayNo.toString() ==
                              "5"
                          ? 'Debit card'
                          : mRefundDetails.refundedPaymentGatewayNo
                                      .toString() ==
                                  "6"
                              ? 'Credit Card'
                              : mRefundDetails.refundedPaymentGatewayNo
      ...
      pw.Expanded(
          ...
          child: pw.Container(
            ...
            child: pw.Align(
              alignment: pw.Alignment.centerRight,
              child: mRefundDetails.refundedPaymentGatewayNo.toString() == "0"
                  ? pw.Text(
                      '$currencyData ${getDoubleValue(roundToNearestPossible(getDoubleValue(getDoubleValue(mRefundDetails.refundedPayAmountCash).toStringAsFixed(2)))).toStringAsFixed(2)}',
                      style: getBoldTextStyle())
                  : pw.Text(
                      '$currencyData ${getDoubleValue(roundToNearestPossible(getDoubleValue(getDoubleValue(mRefundDetails.refundAmountTotal).toStringAsFixed(2)))).toStringAsFixed(2)}',
                      style: getBoldTextStyle()),
            ),
          )),
    ]),
    if (mRefundDetails.refundedPaymentGatewayNo.toString() == "0")
      pw.Row(children: [
        pw.Expanded(
            flex: 2,
            child: pw.Container(
        ...
        pw.Expanded(
          ...
          child: pw.Container(
            ...
            child: pw.Align(
                alignment: pw.Alignment.centerRight,
                child: pw.Text(
                    '$currencyData ${getDoubleValue(roundToNearestPossible(getDoubleValue(getDoubleValue(mRefundDetails.refundedReturnAmountCash).toStringAsFixed(2)))).toStringAsFixed(2)}',
                    style: getBoldTextStyle())),
          ),
        ),
      ])
